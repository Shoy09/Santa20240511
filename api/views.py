    ...
    if request.method == 'POST':
        try:
            ...
            for detalle_item in detalle_data:
                ...
                detalle_item['cantidad'] = detalle_item.get('cantidad')  # Obtener la cantidad desde el detalle
...
class MerluzasistenciaUpdateByCodigoGeneralView(APIView):
    def put(self, request, idcodigogeneral, idlabor):
        try:
            ...
            # Logging adicional
            print(f"Fecha Abierto del registro: {registro_abierto.FechaAbierto}")
            ...
            # Logging adicional
            print(f"Registro encontrado: {asistencia_detalle}")
        ...
        for importar_asistencia in importar_asistencias:
            ...
            for detalle in detalles_importacion:
                detalle_data = {
                    ...
                    "idactividad": detalle["idactividad"],
                    "idlabor": detalle["idlabor"],
                    "idconsumidor": detalle["idconsumidor"],  # Inicializamos el idconsumidor como cadena vacía
                    "cantidad": detalle["cantidad"]
                }
        ...
        for importar_asistencia in importar_asistencias:
            ...
            for detalle in detalles_importacion:
                detalle_data = {
                    ...
                    "idactividad": detalle["idactividad"],
                    "idlabor": detalle["idlabor"],
                    "idconsumidor": detalle["idconsumidor"],  # Utiliza el valor real de idconsumidor
                    "cantidad": detalle["cantidad"],
                    "tipo_empleado": detalle["tipo_empleado"]
                }
...
@api_view(['POST'])
def create_externo_pasivo(request):
    if request.method == 'POST':
        data = request.data
        if not isinstance(data, list):
            return Response({"error": "Se esperaba una lista de objetos JSON"}, status=status.HTTP_400_BAD_REQUEST)

        # Extraer los DNIs del payload
        dnis = [item['dni'] for item in data]

        # Buscar DNIs existentes en la base de datos
        existing_dnis = Externos.objects.filter(dni__in=dnis).values_list('dni', flat=True)

        # Filtrar los datos para omitir los DNIs existentes
        new_data = [item for item in data if item['dni'] not in existing_dnis]

        if not existing_dnis:
            # Caso 1: Todos los DNIs son nuevos
            serializer = ExternosSerializer(data=data, many=True)
            if serializer.is_valid():
                serializer.save()
                return Response({"message": "Trabajadores creados correctamente."}, status=status.HTTP_201_CREATED)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        elif new_data:
            # Caso 2: Algunos DNIs ya existen
            serializer = ExternosSerializer(data=new_data, many=True)
            if serializer.is_valid():
                serializer.save()
                response_message = f"Trabajadores creados correctamente, menos {', '.join(existing_dnis)} que ya existen en la base de datos."
                return Response({"message": response_message}, status=status.HTTP_201_CREATED)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        else:
            # Caso 3: Todos los DNIs ya existen
            response_message = f"Todos los DNIs ya existen en la base de datos: {', '.join(existing_dnis)}"
            return Response({"message": response_message}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
...
@permission_classes([IsAuthenticated])
class EliminarTodosLosTrabajadores(APIView):
    def post(self, request, format=None):
        Externos.objects.all().delete()
        return Response({"message": "Todos los trabajadores han sido eliminados."})
...
class ExternosDetailView(generics.RetrieveAPIView):
    queryset = Externos.objects.all()
    serializer_class = ExternosSerializer
    lookup_field = 'id_general'
#---------------------------------------------------------
...
from rest_framework import status
from .models import ImportarAsistenciaDetalle, Registro
from .serializers import ActualizacionAnchovetaSerializer  # Asegúrate de que esté importado

class ActualizacionAnchoveta(APIView):
    def put(self, request, idcodigogeneral, idlabor):
        # Agregar logs para depuración
        import logging
        logger = logging.getLogger(__name__)

        logger.info('Actualización solicitada para idcodigogeneral: %s, idlabor: %s', idcodigogeneral, idlabor)

        # Obtener el último registro abierto
        ultimo_registro_abierto = Registro.objects.filter(estado='Abierto').order_by('-id').first()
        if not ultimo_registro_abierto:
            logger.error('No hay registro abierto')
            return Response({'error': 'No hay registro abierto'}, status=status.HTTP_404_NOT_FOUND)

        try:
            detalle = ImportarAsistenciaDetalle.objects.get(
                importar_asistencia__fecha=ultimo_registro_abierto.FechaAbierto,
                idcodigogeneral=idcodigogeneral,
                idlabor=idlabor
            )
        except ImportarAsistenciaDetalle.DoesNotExist:
            logger.error('Detalle no encontrado en el último registro abierto')
            return Response({'error': 'Detalle no encontrado en el último registro abierto'}, status=status.HTTP_404_NOT_FOUND)

        serializer = ActualizacionAnchovetaSerializer(detalle, data=request.data)
        if serializer.is_valid():
            serializer.save()
            logger.info('Detalle actualizado correctamente')
            return Response(serializer.data, status=status.HTTP_200_OK)
        logger.error('Error al actualizar el detalle: %s', serializer.errors)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


# views.py
from rest_framework import viewsets
from .models import CondicionesImport
from .serializers import CondicionesImportSerializer

class CondicionesImportViewSet(viewsets.ModelViewSet):
    queryset = CondicionesImport.objects.all()
    serializer_class = CondicionesImportSerializer
